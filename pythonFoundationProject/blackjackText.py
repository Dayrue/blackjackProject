...
while i < 100:
    ...
    if winLogic() != "Blackjack" and not (len(d.hand) == 2 and d.handValue == 21):
        mycursor.execute("INSERT INTO handOutcomes (handIdentifier, dealerValue, playerValue, action, result) VALUES (%s, %s, %s, %s, %s)", (str(d.initialValue) + str(p.initialValue), d.initialValue, p.initialValue, action, winLogic()))
        db.commit()
        i += 1
